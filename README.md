...
**Domain:** `Medical / Research` · **Suite:** `Medical & Research Suite` · **Accent:** `#5eead4`
...
**Medical Literature Ai** is a standalone, browser-based AI workflow app for **Medical / Research**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `clinical_question`<br>Clinical question | text | Yes | — | Affects input: Clinical question. |
...
| `population`<br>population | text | No | — | Affects customization: population. |
| `study_types`<br>study types | text | No | — | Affects customization: study types. |
| `date_range`<br>date range | text | No | — | Affects customization: date range. |
| `inclusion_exclusion_criteria`<br>inclusion/exclusion criteria | text | No | — | Affects customization: inclusion/exclusion criteria. |
| `citation_style`<br>citation style | select | No | APA | Affects customization: citation style. |
| `evidence_level`<br>evidence level | select | No | — | Affects customization: evidence level. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9136
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9136`. |
...
- Export download
- Job history
- PubMed/Crossref/semantic scholar optional
...
```bash
curl http://127.0.0.1:9136/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9136/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9136 uvicorn app.main:app --host 0.0.0.0 --port 9136
```
...
- `plugins/medical-literature-ai.json` for plugin metadata.
